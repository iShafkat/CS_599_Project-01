...
def main():
    ...
    X = dataset[0:500,1:257]
    y = dataset[0:500,0:1]
    y = to_categorical(y)
    #load the Test Dataset
    test_array = loadtxt("zip.test")
    test_label1 = test_array[0:250,0:1]
    test_array = test_array[0:250,1:257]
    ...
    t = np.arange(0., 30., 1)
    plt.plot(t, accuracy_one, 'r', t, accuracy_two,'b', t, accuracy_three, 'g',t, accuracy_four, 'y')
   # plt.plot(t, accuracy_four, 'r')
    plt.xlabel("Training Epochs")
    plt.ylabel("%Correct On Test Data")
    plt.show()
    plt.savefig('cs599_output.png')
 def network_one(X,y,test_label1,test_array):
    accuracy_one, epoch =[],1
    model = Sequential()
    model.add(Dense(10, input_dim=256, activation='sigmoid'))
  ...
  def network_three(X,y,test_label1,test_array):
    ...
    model.add(LocallyConnected2D(64, (3,3), input_shape=(16, 16,1), activation='relu'))
    model.add(LocallyConnected2D(16, (5,5), activation='relu'))
    model.add(Flatten())
    model.add(Dense(10, activation='sigmoid'))
    model.compile(loss=losses.mean_squared_error, optimizer='adam', metrics=['accuracy'])
    while epoch<31:
  ...
  def network_four(X,y,test_label1,test_array):
    ...
    X= X.reshape(X.shape[0], 16, 16,1).astype('float32')
    test_array= test_array.reshape(test_array.shape[0], 16, 16,1).astype('float32')
    model.add(Conv2D(128,(3,3), input_shape=(16,16,1), activation='relu'))
    model.add(LocallyConnected2D(16, (5,5), activation='relu'))
    model.add(Flatten())
    model.add(Dense(10, activation='sigmoid'))
    model.compile(loss=losses.mean_squared_error, optimizer='adam', metrics=['accuracy'])
